# Shared storage between Armada and Android userdata

## Goal
Allow files (ROMs, media, downloads, saves) to be used from both Android and
Armada without duplicating them.
...
When Armada boots, it sees `userdata` as a raw encrypted block device. It cannot
...
- The current installer already shrinks `userdata` to create the Armada root
...
- Armada can mount it at boot, e.g. `/mnt/shared`, and symlink into user home.
...
```bash
# After creating the Armada partition, from remaining free space next to userdata:
parted /dev/block/mmcblk0 -- mkpart shared fat32 <start> <end>
mkfs.exfat -L ARMADA_SHARED /dev/block/mmcblk0pX
```

Armada side (persistent `/etc/fstab` entry):
...
- Armada works with it natively; Steam could even keep compatibility tools here
...
- Android and Armada both see it.
- Same exFAT POSIX limitations as Option A.

### Option D — Mount Android `userdata` (not viable)

Attempting to mount the encrypted `userdata` from Armada fails at the
...
on the Armada root filesystem.

A runtime service (`armada-shared-storage.service`) is provided: at boot it
