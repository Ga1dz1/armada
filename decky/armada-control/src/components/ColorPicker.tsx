...
export function ColorPicker({ label, hex, onChange }: {
  ...
  return (
    <>
      <PanelSectionRow>
        <div className="armada-color-preview-row">
          {label !== undefined && <span className="armada-color-preview-label">{label}</span>}
          <div className="armada-color-swatch" style={{ backgroundColor: `#${hex}` }} />
          <span className="armada-color-preview-hex">#{hex}</span>
        </div>
      </PanelSectionRow>
      <PanelSectionRow>
        <div className="armada-color-picker">
          <div className="armada-color-sv-wrap" style={{ width: SV_WIDTH, height: SV_HEIGHT }}>
            <canvas
              ref={svCanvasRef}
              width={SV_WIDTH}
              height={SV_HEIGHT}
              className="armada-color-sv-canvas"
              onPointerDown={handleSvPointer}
              onPointerMove={(event) => event.buttons === 1 && handleSvPointer(event)}
            />
            <div
              className="armada-color-cursor"
              style={{ left: svCursorX, top: svCursorY, backgroundColor: `#${hex}` }}
            />
          </div>
          <div className="armada-color-hue-wrap" style={{ width: SV_WIDTH, height: HUE_HEIGHT }}>
            <canvas
              ref={hueCanvasRef}
              width={SV_WIDTH}
              height={HUE_HEIGHT}
              className="armada-color-hue-canvas"
              onPointerDown={handleHuePointer}
              onPointerMove={(event) => event.buttons === 1 && handleHuePointer(event)}
            />
            <div className="armada-color-hue-cursor" style={{ left: hueCursorX }} />
          </div>
        </div>
      </PanelSectionRow>
    </>
